import { looseEqual, VNode, DirectiveBinding } from '../../utils'
...
const resetProp = <T>(el: Partial<T>, prop: keyof T) => {
  el[prop] = undefined
  delete el[prop]
}

// Handle targets update
const handleTargets = ({ targets, vnode }: { targets: unknown[]; vnode: VNode }) => {
  targets.forEach(target => {
    if (vnode.context) vnode.context.$root.$emit(EVENT_TOGGLE, target)
  })
}

// Handle directive updates
/* istanbul ignore next: not easy to test */
const handleUpdate = (el: ToggleElement, binding: DirectiveBinding, vnode: VNode) => {
  if (!isBrowser) {
    return
  }
  ...
  if (!looseEqual(getTargets(binding), el[BV_TOGGLE_TARGETS])) {
    ...
    targets.forEach(target => {
      if (vnode.context) vnode.context.$root.$emit(EVENT_STATE_REQUEST, target)
    })
  }
...
export const VBToggle = {
  bind(el: ToggleElement, binding: DirectiveBinding, vnode: VNode) {
    const targets = bindTargets(vnode, binding, listenTypes, handleTargets)
    if (isBrowser && vnode.context && targets.length > 0) {
      ...
      el[BV_TOGGLE] = function toggleDirectiveHandler(id: string, state: boolean) {
  ...
  unbind(el: ToggleElement, binding: DirectiveBinding, vnode: VNode) /* istanbul ignore next */ {
    unbindTargets(vnode, binding, listenTypes)
