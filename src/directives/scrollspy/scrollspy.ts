...
export class ScrollSpy extends Directive<
  ScrollSpyConfig
> /* istanbul ignore next: not easy to test */ {
  public $scroller?: HTMLElement | Window
  ...
  static ParseBindings(bindings: DirectiveBinding): ScrollSpyConfig {
    ...
    const config: Partial<ScrollSpyConfig> = {}
    ...
    if (isString(bindings.value)) {
      ...
      ;(keys(bindings.value) as (keyof ScrollSpyConfig)[])
    ...
    return config as ScrollSpyConfig
  }
  ...
  _listen() {
    ...
    if (scroller && (scroller as HTMLElement).tagName !== 'BODY') {
  ...
  setObservers(on: boolean) {
    ...
    if (on) {
      ...
      this.$obsScroller =
        observeDom(
          this.getScroller() as HTMLElement,
          () => {
            this.handleEvent('mutation')
          },
          {
  ...
  refresh() {
    ...
    const autoMethod =
      scroller !== (scroller as Window).window ? OffsetMethod.POSITION : OffsetMethod.OFFSET
    ...
    ;(selectAll(String(this.$selector), this.$element)
      // Get HREF value
      .map(link => getAttr(link, 'href') as string)
      ...
      .map(href => {
        ...
        const el = select(id, scroller as HTMLElement)
        if (el && isVisible(el)) {
          return {
      ...
      .filter(Boolean) as ({ offset: number; target: string })[])
      // Sort them by their offsets (smallest first)
      .sort((a, b) => a.offset - b.offset)
      .reduce<Dict<boolean>>((memo, item) => {
  ...
  process() {
    ...
    if (!this.$offsets || !this.$targets) return
  ...
  getScroller(): HTMLElement | undefined | Window {
    ...
    let scroller: HTMLElement | VueExtended | string | Window | undefined = this.$config
      ? this.$config.element
      : undefined
    if (!scroller) {
      return undefined
    } else if (isElement((scroller as VueExtended).$el)) {
      scroller = (scroller as VueExtended).$el as HTMLElement
    } else if (isString(scroller)) {
      scroller = select(scroller) as HTMLElement
    }
    if (!scroller) {
      return undefined
    }
    this.$scroller =
      (scroller as HTMLElement).tagName === 'BODY' ? window : (scroller as HTMLElement)
    return this.$scroller
  }

  getScrollTop(): number {
    const scroller = this.getScroller()
    return scroller === window ? scroller.pageYOffset : (scroller as HTMLElement).scrollTop
  }

  getScrollHeight() {
    return (
      (this.getScroller() as HTMLElement).scrollHeight ||
  ...
  getOffsetHeight() {
    ...
    return scroller === window || !scroller
      ? window.innerHeight
      : (getBCR(scroller) as DOMRect).height
  }
  ...
  setActiveState(el: unknown, active: boolean) {
    if (!el) {
      return
    }
    if (active) {
...
export const VBScrollspy = ScrollSpy.GetBvDirective()
