...
export const filterEvent = (evt: Event) => {
  if (!evt || !evt.target) {
  ...
  const label = (el.tagName === 'LABEL' ? el : closest('label', el)) as HTMLLabelElement
  if (label && label.control && !(label.control as HTMLInputElement).disabled) {
