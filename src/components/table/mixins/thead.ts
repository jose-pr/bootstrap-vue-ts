...
import { filterEvent, textSelectionActive, TableField } from '../utils'
import { isSortable } from '../mixins/sorting'
import { isBusy } from './busy'
import { TFootMixin } from './tfoot'

@Component({})
export class THeadMixin extends Mixins(ItemsMixin, NormalizeSlotMixin) {
  @Prop({ default: () => getComponentConfig('BTable', 'headVariant') })
  headVariant!: string
  @Prop() theadClass?: ElementClass
  @Prop() theadTrClass?: ElementClass

  get headClasses() {
    return [this.headVariant ? 'thead-' + this.headVariant : '', this.theadClass]
  }

  fieldClasses(field: TableField): string[] {
  ...
  headClicked(evt: Event, field: TableField, isFoot: boolean) {
    if (isBusy(this) && this.stopIfBusy(evt)) {
  ...
  renderThead(isFoot = false) {
    ...
    const foot = (this as unknown) as TFootMixin
    ...
    const makeCell = (field: TableField, colIndex: number) => {
      let ariaLabel = null
      ...
      const sortAttrs = isSortable(this) ? this.sortTheadThAttrs(field.key, field, isFoot) : {}
      const sortClass = isSortable(this) ? this.sortTheadThClasses(field.key, field, isFoot) : null
    ...
    const $trs = []
    if (isFoot) {
      $trs.push(h('tr', { class: foot.tfootTrClass, attrs: { role: 'row' } }, $cells))
    } else {
      const scope = {
